...
export const getEditorWillMount =
	...
	({
		...
		return (monaco: typeof EditorApi) => {
			...
			completionItemDispose = monaco.languages.registerCompletionItemProvider(
				id,
				{
...
const buildGrammarGraph = (tools: Tools) => {
	...
	const vocabulary: VocabularyPack<typeof lexer, typeof parser> =
		new VocabularyPack(lexer, parser);
	const grammarGraph: GrammarGraph<typeof lexer, typeof parser> =
		new GrammarGraph(vocabulary, grammar);
	return grammarGraph;
};

export const getVtlTheme = (
	name: string
): EditorApi.editor.IStandaloneThemeData => {
	switch (name) {
		case "vtl-default": {
			return {
				base: "vs",
